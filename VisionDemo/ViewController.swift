...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    private let inception = Inceptionv3()
    ...
    func setupVision() {
        guard let visionModel = try? VNCoreMLModel(for: inception.model) else {
            return
        }
    ...
    private func handleClassification(request: VNRequest, error: Error?) {
        ...
        let classifications = results.flatMap { $0 as? VNClassificationObservation }
        if let best = classifications.first {
            DispatchQueue.main.async {
                self.label?.text = best.identifier
            }
        }
    }
    ...
    private func exifOrientation(from deviceOrientation: UIDeviceOrientation) -> CGImagePropertyOrientation {
